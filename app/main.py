...
@app.get("/admin/fix-fecha-pago-tmp")
def admin_fix_fecha_pago_tmp(secret: str = "", dry_run: bool = True):
    """TEMPORAL (2026-08-09): corrige la fecha de pago de facturas de un grupo
    específico de proveedores (RUT hardcodeados abajo) donde el pago quedó
    registrado en un mes distinto al de la factura — artefacto de una carga
    masiva del 2026-07-01. Regla: si `pagos.fecha` y `facturas.fecha_emision`
    no caen en el mismo mes/año, se reemplaza `pagos.fecha` por
    `facturas.fecha_emision`. Solo actúa sobre facturas con EXACTAMENTE un
    pago asociado (evita tocar pagos parciales/múltiples sin revisión manual).

    `dry_run=true` (default) solo informa qué cambiaría, sin escribir nada.
    Llamar una vez con `dry_run=false` para aplicar. Borrar este endpoint
    del código después de usarlo.
    """
    if not _admin_guard(secret):
        return Response(status_code=404)
    ruts = ["96992030-1", "77337752-9", "76496130-7", "76052927-3",
            "96945440-8", "96875230-8", "77630439-5"]
    desde = "2025-11-01"
    conn = db.get_conn()
    try:
        marcadores = ",".join("?" * len(ruts))
        filas = conn.execute(
            f"""
            SELECT f.id AS factura_id, f.folio, f.rut_contraparte, f.fecha_emision,
                   COUNT(p.id) AS n_pagos
            FROM facturas f
            LEFT JOIN pagos p ON p.factura_id = f.id
            WHERE f.tipo='compra' AND f.rut_contraparte IN ({marcadores})
              AND f.fecha_emision >= ?
            GROUP BY f.id
            """,
            (*ruts, desde),
        ...
        for f in filas:
            if f["n_pagos"] != 1:
                if f["n_pagos"] > 1:
                    omitidas.append({
                        "factura_id": f["factura_id"], "folio": f["folio"],
                        "rut": f["rut_contraparte"], "motivo": f"{f['n_pagos']} pagos asociados",
                    })
                continue
            pago = conn.execute(
                "SELECT id, fecha, monto FROM pagos WHERE factura_id=?", (f["factura_id"],)
            ).fetchone()
            fe = f["fecha_emision"] or ""
            pf = pago["fecha"] or ""
            if fe[:7] != pf[:7]:
                cambios.append({
                    "factura_id": f["factura_id"], "folio": f["folio"], "rut": f["rut_contraparte"],
                    "fecha_emision": fe, "pago_id": pago["id"],
                    "fecha_pago_anterior": pf, "fecha_pago_nueva": fe,
                })
                if not dry_run:
                    conn.execute("UPDATE pagos SET fecha=? WHERE id=?", (fe, pago["id"]))
        if not dry_run and cambios:
            conn.commit()
    finally:
        conn.close()
    return JSONResponse({
        "dry_run": dry_run, "total_cambios": len(cambios),
        "cambios": cambios, "omitidas": omitidas,
    })
